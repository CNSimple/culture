...
window.addEventListener('message',event=>{if(event.origin!==window.location.origin)return;const frame=document.querySelector('.home-map-frame');const wrap=document.querySelector('.map-wrap');const side=document.querySelector('.home-layout .sidebar');const assistant=document.querySelector('.home-layout .assistant');const recommend=document.querySelector('.recommend');if(event.data?.type==='yanzhao:heritage-open'&&frame&&wrap){document.body.classList.add('heritage-open');[side,assistant,recommend].forEach(el=>{if(el)el.style.display='none'});wrap.style.cssText='position:fixed;z-index:50;inset:72px 0 0;width:100vw;height:calc(100vh - 72px);min-height:0;border:0;border-radius:0';frame.style.cssText='width:100%;height:100%;border:0'});if(event.data?.type==='yanzhao:heritage-close'&&frame&&wrap){document.body.classList.remove('heritage-open');[side,assistant,recommend].forEach(el=>{if(el)el.style.display=''});wrap.removeAttribute('style');frame.removeAttribute('style')}});
$$('#filters button').forEach(b=>b.addEventListener('click',()=>{$$('#filters button').forEach(x=>x.classList.toggle('selected',x===b));const filter=b.dataset.filter;$$('.map-pin').forEach(x=>x.style.display=(filter==='全部'||filter==='成语')?'':'none');$$('.city').forEach(x=>x.style.opacity=(filter==='全部'||filter==='古迹')?'1':'.35');document.querySelector('.home-map-frame')?.contentWindow?.postMessage({type:'map-filter',filter},'*');}));
