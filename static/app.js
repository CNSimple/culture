...
window.addEventListener('message',event=>{if(event.origin!==window.location.origin)return;const frame=document.querySelector('.home-map-frame');const wrap=document.querySelector('.map-wrap');if(event.data?.type==='yanzhao:heritage-open'&&frame&&wrap){document.body.classList.add('heritage-open');wrap.style.cssText='position:fixed;z-index:50;inset:72px 0 0;width:100vw;height:calc(100vh - 72px);min-height:0;border:0;border-radius:0;box-shadow:0 0 0 100vmax rgba(3,12,14,.58)';frame.style.cssText='width:100%;height:100%;border:0'}if(event.data?.type==='yanzhao:heritage-close'&&frame&&wrap){document.body.classList.remove('heritage-open');wrap.removeAttribute('style');frame.removeAttribute('style')}});
